...
function isNoContentGenerated(response: OpenRouterResponse): boolean {
  ...
  // 2. Images array exists and has at least one image
  return (!content || content.trim() === '') && !hasImages;
}
...
export async function getOpenRouterCompletion(
  ...
  const isImageGeneration = options?.modalities?.includes('image') ?? false;
  // Use different retry config based on modality (image gen is expensive, minimize retries)
  const retryConfig = isImageGeneration ? IMAGE_COMPLETION_RETRY_CONFIG : COMPLETION_RETRY_CONFIG;
  ...
  if (lastError && lastError.message.includes('No content generated')) {
    ...
    throw new ApiErrorResponse(
      `Model ${selectedModel} failed to generate content after ${retryConfig.maxRetries + 1} attempts. This typically occurs when the model is warming up from a cold start.`,
  ...
  throw new ApiErrorResponse(
    `Failed to get completion after ${retryConfig.maxRetries + 1} attempts: ${lastError?.message}`,
