...
-- Result: Frontend could not display saved personality preset value
...
SELECT 
    id,
    (preferences->'model'->>'personality_preset') as personality_preset_value
FROM (
    SELECT get_user_complete_profile(id) as data, id
    FROM profiles
    LIMIT 1
) sub;
