...
-- ============================================================================
-- Purpose: Optimizes ILIKE pattern matching for message content
-- Query pattern: WHERE content ILIKE '%query%' AND role IN ('user', 'assistant')
-- Performance: ~10-50x faster than sequential scan
-- Storage: ~5-10MB per 100,000 messages
-- Note: Filtered to exclude system messages (not user-searchable)
...
-- Includes: content, message_timestamp for result display
-- Performance: Faster joins during message content search
-- Storage: ~3-5MB per 100,000 messages
