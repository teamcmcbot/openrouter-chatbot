...
-- Scope: Public tables flagged by Performance Advisor (see README)
...
BEGIN
  ...
  LOOP
    ...
    -- Normalize any pre-existing wrappers, then reapply desired wrappers (ensures idempotency)
    IF new_using IS NOT NULL THEN
      -- Remove wrappers (normalize)
    ...
    IF changed_using OR changed_check THEN
      ...
      RAISE NOTICE 'Updated policy % on %.% (using_changed=%, check_changed=%)', r.polname, r.schemaname, r.relname, changed_using, changed_check;
    ELSE
