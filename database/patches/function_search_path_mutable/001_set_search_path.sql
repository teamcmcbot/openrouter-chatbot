-- Purpose: Pin search_path for flagged functions to prevent name shadowing/hijacking
-- Safety: Idempotent; only changes function-level search_path. No signature or body changes.
-- Scope: Functions listed by Security Advisor (public schema)
--
-- Roll forward: sets search_path = 'pg_catalog, public'
-- Roll back: run the companion 002_reset_search_path.sql to RESET per function
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Pinned search_path for %.% (%).', fn.schema_name, fn.func_name, fn.identity_args;
  END LOOP;
END$$;
