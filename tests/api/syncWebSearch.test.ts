...
/**
 * Tests for web search flags and URL citations via GET/POST /api/chat/sync
 */
...
jest.mock('/Users/zhenwei.seo/github/openrouter-chatbot/lib/supabase/server', () => {
  ...
  type MessageRow = { id: string; session_id: string; role: string; content: string; model?: string | null; total_tokens?: number; input_tokens?: number; output_tokens?: number; user_message_id?: string | null; content_type?: string | null; elapsed_ms?: number | null; completion_id?: string | null; message_timestamp: string; has_attachments?: boolean; attachment_count?: number; has_websearch?: boolean | null; websearch_result_count?: number | null; error_message?: string | null };
...
describe('Sync API - Web Search Annotations', () => {
  test('POST persists metadata and annotations; GET returns them', async () => {
    // Build a conversation with one assistant message w/ annotations
    const conv = {
      id: 'sess-sync-1',
      title: 'Web search test',
      userId: 'u1',
      messages: [
        { id: 'u-1', role: 'user' as const, content: 'Find sources', timestamp: new Date(), contentType: 'text' as const },
        { id: 'a-1', role: 'assistant' as const, content: 'Here are sources', model: 'm', total_tokens: 10, timestamp: new Date(), contentType: 'text' as const,
          has_websearch: true, websearch_result_count: 2,
          annotations: [
            { type: 'url_citation', url: 'https://example.com/1', title: 'One', content: 'Snippet 1', start_index: 0, end_index: 5 },
            { type: 'url_citation', url: 'https://example.com/2', title: 'Two', content: 'Snippet 2', start_index: 6, end_index: 10 },
          ]
        }
      ],
      messageCount: 2,
      totalTokens: 10,
      lastModel: 'm',
      lastMessagePreview: 'Here are sources',
    ...
    const found = getBody.conversations.find(c => c.id === 'sess-sync-1');
    ...
    expect(a1.annotations.map((a: any) => a.url).sort()).toEqual(['https://example.com/1','https://example.com/2']);
  });
});
