...
/**
 * Tests for web search flags and URL citations persistence via POST /api/chat/messages
 */
...
jest.mock('../../lib/supabase/server', () => {
  ...
  const profiles: ProfileRow[] = [ { id: 'u1', email: 'u@test.dev', full_name: 'Test User', avatar_url: null, default_model: 'gpt', temperature: 0.7, system_prompt: 'You are a helpful AI assistant.', subscription_tier: 'free', credits: 0 } ];
  const annotations: AnnotationRow[] = [];

  function uuid() { return 'id-' + Math.random().toString(36).slice(2, 10); }

  return {
    __mockState: { sessions, messages, profiles, annotations },
        ...
        const api = {
          ...
          insert: (values: Record<string, unknown> | Record<string, unknown>[]) => {
            if (tableName === 'chat_sessions') {
              ...
              sessions.push(row);
              return { data: row, error: null, select: () => ({ single: async () => ({ data: row, error: null }) }) } as any;
            }
          ...
          upsert: (values: Record<string, unknown> | Record<string, unknown>[]) => {
            if (tableName !== 'chat_messages') return { data: null, error: new Error('upsert unsupported') } as any;
            const v = Array.isArray(values) ? values[0] : values;
            ...
            const row: MessageRow = {
              ...
              has_websearch: Boolean(v['has_websearch']),
              websearch_result_count: (v['websearch_result_count'] as number) || 0,
            };
          ...
          update: (values: Record<string, unknown>) => {
            const updateApi = {
              eq: (col: string, val: unknown) => { ctx.filters[col] = val; return updateApi; },
              is: (col: string, val: unknown) => { ctx.filters[col] = val; return updateApi; },
              in: () => updateApi,
...
describe('Messages API - Web Search Annotations', () => {
  test('persists has_websearch, websearch_result_count, and chat_message_annotations', async () => {
    const assistantMsg = {
      id: 'a-1',
      ...
      annotations: [
        { type: 'url_citation', url: 'https://example.com/1', title: 'One', content: 'Snippet 1', start_index: 0, end_index: 10 },
        { type: 'url_citation', url: 'https://example.com/2', title: 'Two', content: 'Snippet 2', start_index: 11, end_index: 20 },
      ],
    };

    const req = makeReq({ url: 'http://localhost/api/chat/messages', method: 'POST', json: async () => ({ messages: [assistantMsg], sessionId: 'sess-web-1' }) });
  ...
  const state = (serverModule as unknown as { __mockState: { messages: any[]; annotations: any[]; sessions: any[] } }).__mockState;
    ...
    const persisted = state.messages.find((m) => m.id === 'a-1');
    expect(persisted).toBeDefined();
    expect(persisted.has_websearch).toBe(true);
    expect(persisted.websearch_result_count).toBe(2);

    // Annotations inserted
    const anns = state.annotations.filter((a) => a.message_id === 'a-1');
    expect(anns).toHaveLength(2);
    expect(anns.map((a: any) => a.url).sort()).toEqual(['https://example.com/1','https://example.com/2']);
  });
});
