...
/**
 * Tests for reasoning fields via GET/POST /api/chat/sync
 */
            ...
            if (tableName === 'chat_messages') {
              ...
              for (const v of arr) {
                ...
                const row: MessageRow = {
                  ...
                  reasoning_details: (v['reasoning_details'] as Record<string, unknown> | null) ?? null,
                };
...
describe('Sync API - Reasoning fields', () => {
  test('POST persists reasoning; GET returns it', async () => {
    const conv = {
      id: 'sess-reason-1',
      title: 'Reasoning test',
      userId: 'u1',
      messages: [
        { id: 'u-1', role: 'user' as const, content: 'Solve this', timestamp: new Date(), contentType: 'text' as const },
        { id: 'a-1', role: 'assistant' as const, content: 'Answer', model: 'm', total_tokens: 42, timestamp: new Date(), contentType: 'text' as const,
          reasoning: 'I think step by step...',
          reasoning_details: { blocks: [{ type: 'thought', text: 'step' }] }
        }
      ],
      messageCount: 2,
      totalTokens: 42,
  ...
  const getReq = makeReq({ url: 'http://localhost/api/chat/sync?summary_only=false', method: 'GET' });
    ...
    const found = getBody.conversations.find(c => c.id === 'sess-reason-1');
    expect(found).toBeDefined();
    const a1 = found.messages.find((m: any) => m.id === 'a-1');
    expect(a1).toBeDefined();
    expect(a1.reasoning).toBe('I think step by step...');
    expect(a1.reasoning_details).toBeDefined();
